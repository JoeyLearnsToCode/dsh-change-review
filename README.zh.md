...
- **右键上下文菜单** — 审查页文件列表与轮次变更卡片（文件行 + 产物 chips）均支持右键菜单：「打开文件」（在选定编辑器或预览面板中打开）与「复制路径」
- **编辑器选择器** — 在会话头部（会话日志按钮左侧）新增代码编辑器选择器。自动检测已安装的编辑器（VS Code、Cursor、Sublime Text、Vim 等），通过 `which` 命令和 `/Applications` 路径识别。选中编辑器后，所有「打开文件」操作均使用该编辑器打开；未选择时使用系统默认处理程序。选择保存在 localStorage。新增服务端路由 `/diff-review/open-with-editor` 在主机端派发编辑器命令。
