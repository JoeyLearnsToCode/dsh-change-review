...
| 右侧栏集成（内置） | dsh webui 自带右侧栏时，注册「修改审查」tab 类型：在右侧栏「开始」页出现入口胶囊，点击即打开（同一套文件列表 / diff / 撤回，跟随当前会话），与对话「审查」标签并存。客户端无该服务时静默跳过，其余功能不受影响 |
| 侧边栏集成（better-sidebar） | 安装 dsh-better-sidebar 时，自动向其侧边栏注册「修改审查」tab（`+` 菜单打开、单实例），与对话「审查」标签并存；侧边卡片设置页自动出现启用/禁用开关。未安装该插件时本功能静默跳过，其余功能不受影响 |
...
2. 左侧文件列表（每个文件的写入/编辑次数、新增/删除行数以图标**逐类一行**显示），右侧选中文件的 diff 对比。左右两栏**独立滚动**（滚动文件列表不会带动 diff 预览，反之亦然），浏览文件时预览位置保持不动。两栏之间的**分隔条可左右拖动**调整宽度（支持方向键微调、双击恢复默认，宽度记在 localStorage），窄屏（≤768px）自动改为主从式单栏。顶部可切换审查范围：**「此会话」**（会话累计）或 **「最新一轮」**（最近一轮对话的修改）
...
7. **右侧栏审查**：展开右侧栏，「开始」页里的 **修改审查** 胶囊即可打开审查页（同一套文件列表 + diff + 撤回能力，跟随当前会话）；与对话「审查」标签互相独立、可并存。注意：注册该胶囊后，右侧栏入口变为 2 个，展开时的默认页会由「文件」变为「开始」（dsh 内置规则）
8. **侧边栏审查（better-sidebar）**：安装了 dsh-better-sidebar 时，其侧边栏 `+` 菜单会出现「修改审查」页；与对话「审查」标签互相独立、可并存，侧边卡片设置页可关闭
...
### v0.5.0 — dsh 内置右侧栏集成（2026-09-14）

- **内置右侧栏 tab** — dsh webui 自带右侧栏（`@deepseek-ai/dsh-client-ui-sidebar-right`）时，按其公开的两段式扩展路径注册「修改审查」tab 类型：类型进 `ctx.sidebarRightTabs`（含「开始」页的入口胶囊），body 与 chip 标题进 `sidebar.right.pane.tab(.title)` 座位，key 为类型 id。复用对话「审查」标签的完整视图（文件列表 / diff / 撤回 / 会话·最新一轮切换），跟随当前会话，与对话「审查」标签并存
- **可选软依赖** — 同样经 `ctx.inject(["sidebarRightTabs"])` 延迟绑定：客户端未提供该服务时整段静默跳过，本插件其余功能不受影响
- **已知影响** — 入口胶囊注册后，右侧栏「开始」页入口变为 2 个，展开时的默认页由「文件」变为「开始」（dsh 内置种子规则）

### v0.4.0 — better-sidebar 侧边栏集成（2026-08-19）

- **侧边栏 tab** — 安装 dsh-better-sidebar（v0.4.0+ 的 `ctx.betterSidebar` 服务）时，向其侧边栏注册「修改审查」tab（`+` 菜单可见、单实例、跟随当前会话），复用对话「审查」标签的完整视图（文件列表 / diff / 撤回 / 会话·最新一轮切换）。作为可选软依赖实现：未安装 better-sidebar 时静默跳过，本插件其余功能不受影响
- **免费开关** — 侧边卡片设置页自动出现该 tab 的启用/禁用开关，无需额外配置
...
- **右键上下文菜单** — 审查页文件列表与轮次变更卡片（文件行 + 产物 chips）均支持右键菜单：「打开文件」（在选定编辑器或预览面板中打开）与「在文件资源管理器中展示」
...
- **在文件资源管理器中展示** — 右键菜单新增「在文件资源管理器中展示」，直接调用 `open -R`（macOS）/ `explorer`（Windows）/ `xdg-open`（Linux）在文件管理器中高亮文件。
...
Browser UI（lib/client.js，__ModuleLoader__ bundle）
  · 会话头部探针同步当前会话（隐藏）
  · 「审查」视图标签 + 角标
  · 内置右侧栏集成：ctx.sidebarRightTabs 注册「修改审查」tab 类型 + 座位 body（可选软依赖，ctx.inject 延迟绑定）
  · better-sidebar 集成：ctx.betterSidebar 注册侧边栏「修改审查」tab（可选软依赖，ctx.inject 延迟绑定）
  · 设置页「修改审查」颜色自定义
  · EventSource 实时订阅
```
