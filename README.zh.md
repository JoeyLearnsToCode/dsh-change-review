...
| 侧边栏集成 | 安装 dsh-better-sidebar 时，自动向其侧边栏注册「修改审查」tab（`+` 菜单打开、单实例），与对话「审查」标签并存；侧边卡片设置页自动出现启用/禁用开关。未安装该插件时本功能静默跳过，其余功能不受影响 |
...
7. **侧边栏审查**：安装了 dsh-better-sidebar 时，其侧边栏 `+` 菜单会出现「修改审查」页（同一套文件列表 + diff + 撤回能力，跟随当前会话）；与对话「审查」标签互相独立、可并存，侧边卡片设置页可关闭
