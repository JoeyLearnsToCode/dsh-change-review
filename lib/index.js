...
// 临时开关：屏蔽“编辑器检测/选择”全系功能（/diff-review/editors、open-with-editor、
// reveal、editor-icon 四个路由）。原因：Windows + pm2 以 node 方式运行时，页面刷新会
// 触发 detectEditors() 连续 spawn 约 26 个 cmd.exe 探测 `which`，曾导致宿主进程被误杀、
// pm2 无限重启。现仅保留“系统默认编辑器打开文件”（宿主 workspaces.openPath 提供）。
// 代码暂不删除，后续恢复时把此开关翻回 false 即可。
...
function apply(ctx) {
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (req, res) => {
      // 已屏蔽（EDITOR_FEATURES_DISABLED）：不再执行 detectEditors 的 which 探测。
      if (EDITOR_FEATURES_DISABLED) return sendJson(res, 200, { editors: [] })
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      // 已屏蔽（EDITOR_FEATURES_DISABLED）：不再用 execSync 启动指定编辑器。
      if (EDITOR_FEATURES_DISABLED) return sendJson(res, 400, { ok: false, error: '编辑器选择功能已停用' })
      try {
        const body = await readJsonBody(req)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: async (req, res) => {
      // 已屏蔽（EDITOR_FEATURES_DISABLED）：不再用 execSync 调用 explorer/open 展示文件。
      if (EDITOR_FEATURES_DISABLED) return sendJson(res, 200, { ok: true })
      try {
        const body = await readJsonBody(req)
  ...
  ctx.effect(() => ctx.webServer.register({
    ...
    handler: (req, res) => {
      // 已屏蔽（EDITOR_FEATURES_DISABLED）：编辑器图标不再提供。
      if (EDITOR_FEATURES_DISABLED) { res.statusCode = 404; res.end('disabled'); return }
      try {
