...
function detectEditors() {
  const candidates = [
    { id: 'vscode', name: 'Visual Studio Code', command: 'code', openTemplate: '{cmd} --goto {file}:{line}:{col}', check: () => { try { execSync('which code', {stdio:'pipe'}); return true } catch(e) { return existsSync('/Applications/Visual Studio Code.app/Contents/Resources/app/bin/code') } } },
    { id: 'cursor', name: 'Cursor', command: 'cursor', openTemplate: '{cmd} --goto {file}:{line}:{col}', check: () => { try { execSync('which cursor', {stdio:'pipe'}); return true } catch(e) { return existsSync('/Applications/Cursor.app/Contents/MacOS/Cursor') } } },
    { id: 'windsurf', name: 'Windsurf', command: 'windsurf', openTemplate: '{cmd} {file}', check: () => { try { execSync('which windsurf', {stdio:'pipe'}); return true } catch(e) { return existsSync('/Applications/Windsurf.app/Contents/MacOS/windsurf') } } },
    { id: 'webstorm', name: 'WebStorm', command: 'webstorm', openTemplate: '{cmd} {file}', check: () => { try { execSync('which webstorm', {stdio:'pipe'}); return true } catch(e) {} return false } },
    { id: 'sublime', name: 'Sublime Text', command: 'subl', openTemplate: '{cmd} {file}:{line}:{col}', check: () => { try { execSync('which subl', {stdio:'pipe'}); return true } catch(e) {} return false } },
    { id: 'bbedit', name: 'BBEdit', command: 'bbedit', openTemplate: '{cmd} {file}', check: () => { try { execSync('which bbedit', {stdio:'pipe'}); return true } catch(e) {} return false } },
    { id: 'mate', name: 'TextMate', command: 'mate', openTemplate: '{cmd} {file}', check: () => { try { execSync('which mate', {stdio:'pipe'}); return true } catch(e) {} return false } },
    { id: 'vim', name: 'Vim', command: 'vim', openTemplate: '{cmd} {file}', check: () => { try { execSync('which vim', {stdio:'pipe'}); return true } catch(e) {} return false } },
    { id: 'nvim', name: 'Neovim', command: 'nvim', openTemplate: '{cmd} {file}', check: () => { try { execSync('which nvim', {stdio:'pipe'}); return true } catch(e) {} return false } },
  ]
  return candidates.map((c) => ({ id: c.id, name: c.name, command: c.command, openTemplate: c.openTemplate, detected: c.check() }))
}
