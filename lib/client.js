window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		function revealInFinderFor(sessionId, path) {
			const abs = resolveAbsPath(sessionId, path);
			apiReveal(abs).catch(() => {});
		}
				...
				return React.createElement("button", {
					...
					onContextMenu: (e) => {
						...
						setMenu({ x: e.clientX, y: e.clientY, items: [
							...
							{ label: "在 Finder 中展示", run: () => { const sid = store.currentSession; if (sid) revealInFinderFor(sid, f.path); } }
						]});
					}
				},
		...
		function TurnReview({ matched, sessionId, turn: turnLoc, seq, openFile }) {
			...
			const showCtx = (e, path) => {
				...
				setMenu({ x: e.clientX, y: e.clientY, items: [
					...
					{ label: "在 Finder 中展示", run: () => { if (sessionId) revealInFinderFor(sessionId, path); } }
				]});
			};
		...
		const EDITOR_ICONS = {
			vscode: "🟦", cursor: "🟢", windsurf: "🌊", zed: "⚡", 'vscode-insiders': "🟦",
			'vscodium': "🟦",
			idea: "🟧", 'idea-community': "🟧", pycharm: "🟩", 'pycharm-ce': "🟩",
			webstorm: "🔵", goland: "🔵", datagrip: "🟢", phpstorm: "🟣",
			rubymine: "🔴", clion: "🟡", rustrover: "🟠",
			'android-studio': "🤖", fleet: "🚀",
			xcode: "🔨",
			sublime: "📝", bbedit: "📄", mate: "📄", nova: "✨", coteditor: "📝",
			vim: "⌨️", nvim: "⌨️", emacs: "✝️", nano: "📟", textedit: "📄",
		};
		function EditorPicker(props) {
			...
			const icon = selectedEditor ? (EDITOR_ICONS[selectedEditor.id] || "") : "";
			...
			return React.createElement("div", { className: "drv-editor", ref: rootRef },
				React.createElement("button", {
					...
					React.createElement("span", { className: "drv-editor-label" }, editorLoading ? "检测中…" : (icon ? icon + " " + label : label)),
				...
				open ? React.createElement("div", { className: "drv-editor-menu" },
					...
					detected.map((ed) => {
						const edIcon = EDITOR_ICONS[ed.id] || "";
						return React.createElement("button", {
							type: "button",
							key: ed.id,
