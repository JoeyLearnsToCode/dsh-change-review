window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		// 打开编辑器 / 在文件管理器中展示 都作用于 dsh 所在机器，只有本机
		//（loopback，含 dsh.localhost）访问才有意义。远程访问时隐藏这些入口。
		function isLoopbackHost(hostname) {
			const h = String(hostname || "").toLowerCase().replace(/^\[|\]$/g, "");
			return h === "localhost" || h === "::1" || /^127\./.test(h) || h.endsWith(".localhost");
		}
		const LOCAL_HOST = isLoopbackHost(window.location.hostname);
		...
		function openFileFor(sessionId, path, cwd) {
			try {
				...
				// selectedEditor 记在 localStorage，远程访问时即使有残留也不走服务端打开
				const ed = LOCAL_HOST ? store.selectedEditor : null
				if (ed && ed.id) {
		...
		function openViaWorkspace(abs) {
			try {
				if (!abs) return;
				// 系统默认：走插件自带的 /diff-review/open-default 路由（服务端 shell 调用）。
				// 插件 ctx.workspaces 面里没有 openPath（dsh 只注入了 list），所以不能再指望它。
				apiOpenDefault(abs).then((v) => {
					if (v && v.ok) return;
					if (openViaCtxWorkspace(abs)) return;
					// 两个通道都失败时明确报错——静默失败就是“什么也没发生”的来源。
					window.alert("打开文件失败：" + ((v && v.error) || "未知错误"));
				}).catch(() => openViaCtxWorkspace(abs));
			} catch (e) {}
		}
		/** @returns {boolean} 是否真的把请求交给了 dsh 运行时。 */
		function openViaCtxWorkspace(abs) {
			try {
		...
		function apiOpenDefault(path) {
			return fetch("/diff-review/open-default", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ path })
		...
		let editorsRequestedAt = 0
		function loadEditors() {
			if (!LOCAL_HOST) return; // 非本机访问不探测、不请求
			// apply() and every EditorPicker mount ask for the same list; collapse the
			// duplicates so a reload storm can't fan out into bursts of requests.
			const now = Date.now();
			if (store.editorLoading || (store.editors.length && now - editorsRequestedAt < 60000)) return;
			editorsRequestedAt = now;
		...
		function setMode(mode) {
			setState({ mode: mode, selected: null, detail: null, mobilePane: "list" });
		...
		function Detail({ onRevert, onRevertAll, busy }) {
			...
			return React.createElement("div", null,
				React.createElement("div", { className: "drv-detail-toolbar" },
					React.createElement("button", { type: "button", className: "drv-btn drv-back-btn", title: "返回文件列表", onClick: () => setState({ mobilePane: "list" }) }, "← 文件列表"),
		...
		function CtxMenu({ menu, onClose }) {
			...
			return React.createElement("div", {
				className: "drv-ctx-menu",
				style: { left: Math.min(menu.x, window.innerWidth - 150), top: Math.min(menu.y, window.innerHeight - 80) },
				onClick: (e) => e.stopPropagation()
			}, (menu.items || []).map((it, i) =>
				React.createElement("button", { key: i, className: "drv-ctx-menu-item", onClick: () => { it.run(); onClose(); } }, it.label)));
		}

		// ── 变更统计：一类一行，类别用图标表示，尽量少占横向宽度 ────────
		function MetaIcon(props) {
			const d = props.kind === "write"
				? "M9.2 2.5H4.4a.9.9 0 0 0-.9.9v9.2a.9.9 0 0 0 .9.9h7.2a.9.9 0 0 0 .9-.9V6.2z M9.2 2.5v3.7h3.3 M5.8 9h4.4M5.8 11.5h2.9"
				: props.kind === "edit"
				? "M10.9 2.6l2.5 2.5-7.3 7.3-3 .8.8-3z M9.7 3.8l2.5 2.5"
				: props.kind === "add"
				? "M8 3.8v8.4M3.8 8h8.4"
				: "M3.8 8h8.4";
			return React.createElement("svg", {
				className: "drv-meta-icon", width: props.size, height: props.size,
				viewBox: "0 0 16 16", fill: "none", "aria-hidden": "true", focusable: "false"
			}, React.createElement("path", {
				d: d, stroke: "currentColor", strokeWidth: 1.5,
		...
		function FileMeta(props) {
			...
			const f = props.file;
			const rows = [];
			if (f.writes > 0) rows.push({ kind: "write", label: "写入", n: f.writes });
			if (f.edits > 0) rows.push({ kind: "edit", label: "编辑", n: f.edits });
			if (f.added > 0) rows.push({ kind: "add", label: "新增行", n: f.added, color: colors.addFg });
			if (f.removed > 0) rows.push({ kind: "del", label: "删除行", n: f.removed, color: colors.delFg });
			if (rows.length === 0) return null;
			return React.createElement("span", {
				className: "drv-file-meta" + (props.inline ? " drv-file-meta-inline" : "")
			}, rows.map((r) => React.createElement("span", {
				key: r.kind, className: "drv-meta-row", title: r.label + " " + r.n,
				style: r.color ? { color: r.color } : null
			},
				React.createElement("span", { className: "drv-sr-only" }, r.label),
				React.createElement(MetaIcon, { kind: r.kind, size: 12 }),
		...
		function FileList({ openFile }) {
			...
			const [onlyWorkspace, setOnlyWorkspace] = React.useState(loadOnlyWorkspace);
			if (loading) return React.createElement("div", { className: "drv-empty" }, "加载中…");
			...
			const fileRow = (f) => {
				...
				return React.createElement("button", {
					...
					// 非本机访问时不提供右键菜单（打开/展示都只对本机有意义）
					onContextMenu: LOCAL_HOST ? (e) => {
						e.preventDefault(); e.stopPropagation();
						setMenu({ x: e.clientX, y: e.clientY, items: [
							{ label: "打开文件", run: () => { if (openFile) openFile(f.path, f.cwd); } },
							{ label: "在文件资源管理器中展示", run: () => { const sid = store.currentSession; if (sid) revealInFinderFor(sid, f.path, f.cwd); } }
						]});
					} : undefined
				},
					React.createElement("span", { className: "drv-file-name" }, f.name),
					f.outside ? React.createElement("span", { className: "drv-file-outside", title: "工作区外：" + (f.path || f.name) }, "外部") : null,
					React.createElement(FileMeta, { file: f }));
			};
			const items = list || [];
			const outsideCount = items.filter((f) => f.outside).length;
			const shown = onlyWorkspace ? items.filter((f) => !f.outside) : items;
			return React.createElement("div", null,
				outsideCount > 0 ? React.createElement("label", {
					className: "drv-filelist-filter",
					title: "工作区外的文件（其它仓库、临时目录等）同样会出现在审查列表里，可按需隐藏"
				}, React.createElement("input", {
					type: "checkbox",
					checked: onlyWorkspace,
					onChange: (e) => { setOnlyWorkspace(e.target.checked); saveOnlyWorkspace(e.target.checked); }
				}), "仅工作区内（" + outsideCount + " 个在工作区外）") : null,
				shown.length === 0 ? React.createElement("div", { className: "drv-empty" }, "已开启「仅工作区内」，" + outsideCount + " 个工作区外的修改已隐藏") : shown.map(fileRow),
		...
		function SessionProbe(props) {
			React.useEffect(() => {
				if (props.sessionId && store.currentSession !== props.sessionId) {
					...
					setState({ currentSession: props.sessionId, files: null, selected: null, detail: null, mode: "session", turnData: null, latestTurn: 0, error: null, loadingFiles: true, mobilePane: "list" });
					refreshFromServer();
				}
			}, [props.sessionId]);
			return null;
		...
		function TurnReview({ matched, sessionId, turn: turnLoc, seq, openFile }) {
			...
			// 非本机访问时不提供右键菜单（打开/展示都只对本机有意义）
			const showCtx = LOCAL_HOST ? (e, path, cwd) => {
				e.preventDefault(); e.stopPropagation();
				setMenu({ x: e.clientX, y: e.clientY, items: [
					{ label: "打开文件", run: () => { if (sessionId) openFileFor(sessionId, path, cwd); } },
					{ label: "在文件资源管理器中展示", run: () => { if (sessionId) revealInFinderFor(sessionId, path, cwd); } }
				]});
					...
					data.files.map((f) => {
						...
						return React.createElement("div", { key: f.path, className: "drv-turn-file" },
							React.createElement("button", {
								...
								React.createElement("span", { className: "drv-turn-file-name", title: f.path || f.name }, f.name),
								f.outside ? React.createElement("span", { className: "drv-file-outside", title: "工作区外：" + (f.path || f.name) }, "外部") : null,
								React.createElement(FileMeta, { file: f, inline: true }),
								React.createElement("span", { className: "drv-header-spacer" }),
		...
		// ── 清单/详情之间的可拖动分隔条（宽度持久化到 localStorage） ──────
		const FILELIST_W_KEY = "dsh.diff-review.filelistWidth";
		const FILELIST_W_DEFAULT = 250;
		const FILELIST_W_MIN = 140;
		const FILELIST_W_MAX = 640; // 仅用于读回存档：拖动时按容器实时宽度收敛
		const FILELIST_W_STEP = 16;
		const FILELIST_W_GAP = 200; // 详情面板至少保留的宽度
		// 工作区外的文件（其它仓库 / 临时目录）默认一并展示，可持久化地只看工作区内的
		const ONLY_WS_KEY = "dsh.diff-review.onlyWorkspace";
		function loadOnlyWorkspace() {
			try { return localStorage.getItem(ONLY_WS_KEY) === "1"; } catch (e) { return false; }
		}
		function saveOnlyWorkspace(v) {
			try { localStorage.setItem(ONLY_WS_KEY, v ? "1" : "0"); } catch (e) {}
		}
		function loadFilelistWidth() {
			try {
				const raw = Number(localStorage.getItem(FILELIST_W_KEY));
				if (!Number.isFinite(raw)) return FILELIST_W_DEFAULT;
				return Math.min(Math.max(raw, FILELIST_W_MIN), FILELIST_W_MAX);
			} catch (e) {
				return FILELIST_W_DEFAULT;
			}
		}
		function saveFilelistWidth(w) {
			try {
				localStorage.setItem(FILELIST_W_KEY, String(Math.round(w)));
			} catch (e) {}
		}
		function Splitter(props) {
			const start = React.useRef(null);
			const clamp = (w, max) => Math.max(FILELIST_W_MIN, Math.min(w, Math.max(FILELIST_W_MIN, max)));
			const onMouseDown = (e) => {
				e.preventDefault();
				start.current = { x: e.clientX, w: props.width, max: e.currentTarget.parentNode.clientWidth - FILELIST_W_GAP };
				const move = (ev) => {
					if (!start.current) return;
					props.onResize(clamp(start.current.w + (ev.clientX - start.current.x), start.current.max));
				};
				const up = () => {
					start.current = null;
					window.removeEventListener("mousemove", move);
					window.removeEventListener("mouseup", up);
					document.body.classList.remove("drv-resizing");
				};
				window.addEventListener("mousemove", move);
				window.addEventListener("mouseup", up);
				document.body.classList.add("drv-resizing");
			};
			const onKeyDown = (e) => {
				if (e.key !== "ArrowLeft" && e.key !== "ArrowRight") return;
				e.preventDefault();
				const max = e.currentTarget.parentNode.clientWidth - FILELIST_W_GAP;
				props.onResize(clamp(props.width + (e.key === "ArrowRight" ? FILELIST_W_STEP : -FILELIST_W_STEP), max));
			};
			return React.createElement("div", {
				className: "drv-splitter",
				role: "separator",
				...
				onKeyDown: onKeyDown,
				onDoubleClick: () => props.onResize(FILELIST_W_DEFAULT)
			});
		}
		...
		function ReviewView(props) {
			...
			const mode = useStore((s) => s.mode);
			const turnData = useStore((s) => s.turnData);
			const pane = useStore((s) => s.mobilePane);
			const count = mode === "latest" ? (((turnData && turnData.files) || []).length) : (files ? files.length : 0);
			const [busy, setBusy] = React.useState(false);
			const [notice, setNotice] = React.useState(null);
			const [filelistWidth, setFilelistWidth] = React.useState(loadFilelistWidth);
			React.useEffect(() => { saveFilelistWidth(filelistWidth); }, [filelistWidth]);
			...
			const doRevert = (op) => {
				...
				apiRevert(session, path, op).then((v) => {
					if (v && v.ok) {
						...
						if (op === null) setState({ selected: null, detail: null, mobilePane: "list" });
						refresh();
					} else {
						window.alert("撤回失败：" + ((v && v.error) || "未知错误"));
					}
			...
			return React.createElement("div", { className: "drv-view" },
				React.createElement("div", { className: "drv-view-header" },
					...
					React.createElement("span", { className: "drv-header-spacer" }),
					// 编辑器下拉框只在审查 tab 内出现（不再注入会话头部插槽）
					LOCAL_HOST ? React.createElement(EditorPicker, null) : null,
					...
					React.createElement("button", {
						...
						onClick: () => { apiClear(store.currentSession).then(() => { setState({ files: [], detail: null, selected: null, turnData: null, latestTurn: 0, mobilePane: "list" }); }); }
					}, "清空")),
				React.createElement("div", {
					className: "drv-view-body drv-pane-" + pane,
					style: { "--drv-filelist-w": filelistWidth + "px" }
				},
					React.createElement("div", { className: "drv-filelist" }, React.createElement(FileList, { openFile: props.openFile })),
					React.createElement(Splitter, { width: filelistWidth, onResize: setFilelistWidth }),
		...
		// ── better-sidebar tab 图标：两栏 diff 字形，与内置图标同风格
		//（16×16、1.5/1.25px 描边、currentColor；区别于内置 diff tab 的单加单减布局）──
		function ReviewTabIcon(props) {
			return React.createElement("svg", { width: props.size, height: props.size, viewBox: "0 0 16 16", fill: "none" },
				React.createElement("rect", { x: 1.5, y: 1.5, width: 13, height: 13, rx: 2.5, stroke: "currentColor", strokeWidth: 1.5 }),
				React.createElement("path", { d: "M8 3.5v9", stroke: "currentColor", strokeWidth: 1.5, strokeLinecap: "round" }),
				React.createElement("path", { d: "M4.25 6.5h2.5M5.5 5.25v2.5", stroke: "currentColor", strokeWidth: 1.25, strokeLinecap: "round" }),
				React.createElement("path", { d: "M9.5 9.5h2.5", stroke: "currentColor", strokeWidth: 1.25, strokeLinecap: "round" }));
		}
...
.drv-filelist { width:var(--drv-filelist-w,250px); overflow:auto; overscroll-behavior:contain; flex-shrink:0; padding:6px 0; }
.drv-splitter { flex:none; width:5px; cursor:col-resize; background:transparent; border-left:1px solid rgba(128,128,128,0.3); }
.drv-splitter:hover, .drv-splitter:focus-visible { background:rgba(80,120,255,0.4); outline:none; }
.drv-resizing, .drv-resizing * { cursor:col-resize; user-select:none; }
.drv-file { display:flex; align-items:flex-start; gap:6px; width:100%; padding:6px 10px; cursor:pointer; border:none; background:transparent; color:inherit; text-align:left; font-family:inherit; font-size:12.5px; }
.drv-file:hover { background:rgba(128,128,128,0.12); }
.drv-file.drv-selected { background:rgba(80,120,255,0.18); }
.drv-file-name { font-weight:500; word-break:break-all; flex:1 1 auto; min-width:0; }
.drv-file-meta { font-size:11px; opacity:0.75; flex:none; display:flex; flex-direction:column; align-items:flex-end; gap:1px; }
.drv-file-outside { flex:none; align-self:center; font-size:10.5px; line-height:15px; padding:0 4px; border-radius:3px; border:1px solid rgba(128,128,128,0.45); opacity:0.75; white-space:nowrap; }
.drv-filelist-filter { display:flex; align-items:center; gap:5px; padding:6px 10px; font-size:11px; opacity:0.8; cursor:pointer; user-select:none; }
.drv-file-meta-inline { flex-direction:row; align-items:center; gap:6px; }
.drv-meta-row { display:inline-flex; align-items:center; gap:2px; white-space:nowrap; }
.drv-meta-icon { display:block; flex:none; }
.drv-sr-only { position:absolute; width:1px; height:1px; margin:-1px; padding:0; overflow:hidden; clip:rect(0 0 0 0); white-space:nowrap; border:0; }
...
.drv-gutter { box-sizing:border-box; flex:0 0 34px; text-align:right; padding:0 4px; user-select:none; opacity:0.9; }
.drv-gutter-sign { box-sizing:border-box; flex:0 0 14px; text-align:center; padding:0; }
...
.drv-ctx-menu { position:fixed; z-index:20000; min-width:150px; padding:4px; border:1px solid rgba(128,128,128,0.45); border-radius:8px; background:var(--dsw-alias-bg-overlay, Canvas); color:var(--dsw-alias-label-primary, CanvasText); box-shadow:var(--dsw-shadow-lv3, 0 6px 18px rgba(0,0,0,0.35)); }
.drv-ctx-menu-item { display:block; width:100%; border:none; background:transparent; color:inherit; text-align:left; padding:6px 10px; border-radius:6px; font-size:12px; font-family:inherit; cursor:pointer; }
.drv-ctx-menu-item:hover { background:rgba(80,120,255,0.28); }
...
.drv-editor-menu { position:absolute; top:calc(100% + 4px); right:0; z-index:20000; min-width:170px; padding:4px; border:1px solid rgba(128,128,128,0.45); border-radius:8px; background:var(--dsw-alias-bg-overlay, Canvas); color:var(--dsw-alias-label-primary, CanvasText); box-shadow:var(--dsw-shadow-lv3, 0 6px 18px rgba(0,0,0,0.35)); max-height:260px; overflow:auto; }
...
/* ── 窄屏（≤768px）：清单/详情切换为主从式单栏，清单不再固定 250px 挤占 diff ── */
.drv-back-btn { display:none; flex:none; }
@media (max-width: 768px) {
	.drv-view { padding:8px 10px; }
	.drv-view-header { flex-wrap:wrap; row-gap:6px; }
	.drv-view-body { margin-top:8px; }
	.drv-pane-list .drv-filelist { width:100%; }
	.drv-pane-list .drv-detail { display:none; }
	.drv-pane-detail .drv-filelist { display:none; }
	.drv-splitter { display:none; }
	.drv-back-btn { display:inline-flex; align-items:center; gap:4px; }
	.drv-file { min-height:44px; flex-wrap:wrap; align-content:center; }
	.drv-detail { padding:8px; }
	.drv-gutter { flex:0 0 30px; }
	.drv-gutter-sign { flex:0 0 12px; }
}
`;
		// ── 内置右侧栏 tab 类型（dsh 自带侧栏，与 better-sidebar 相互独立）──────
		// id 同时是 body / 标题座位的注册 key；kind 是 ctx.sidebarRight.openTab 的名字。
		const RIGHT_TAB_ID = "dsh-change-review";
		const RIGHT_TAB_KIND = "dsh-change-review:review";
		...
		function apply(ctx) {
			...
			// 编辑器下拉框仅本机（loopback）可用：远程访问时不请求、不注入
			if (LOCAL_HOST) loadEditors();
			refreshFromServer();
			...
			// ── better-sidebar 集成（可选软依赖：未安装 dsh-better-sidebar 时静默跳过）──
			// 经 ctx.inject 延迟绑定：better-sidebar 的 client 半提供 'betterSidebar' 服务后
			// 才注册侧边栏「修改审查」tab。不能声明在插件级 inject 上：服务缺失会让整个
			// 插件挂起，进而被 web boot 的激活审计判为启动失败。
			ctx.inject(["betterSidebar"], (sctx) => {
				sctx.effect(() => sctx.betterSidebar.registerTab({
					id: "dsh-change-review:review",
					title: () => "修改审查",
					icon: (size) => React.createElement(ReviewTabIcon, { size: size }),
					order: 60,
					single: true,
					component: ({ scope }) => React.createElement(ReviewView, {
						sessionId: scope.sessionId,
						openFile: (path, cwd) => openFileFor(scope.sessionId, path, cwd)
					})
				}), "diff-review: better-sidebar tab");
			});
			// ── dsh 内置右侧栏集成（可选软依赖：客户端没有该服务时静默跳过）────────
			// 与 better-sidebar 同样经 ctx.inject 延迟绑定，理由同上（服务缺失不能拖垮
			// 整个插件）。两段式注册：① 类型进 ctx.sidebarRightTabs（含「开始」页的
			// 入口胶囊，用户点它才打开这个 tab）；② body 与 chip 标题进同名 keyed
			// 座位，key 就是类型的 id。
			ctx.inject(["sidebarRightTabs"], (sctx) => {
				sctx.effect(() => sctx.sidebarRightTabs.register({
					id: RIGHT_TAB_ID,
					kind: RIGHT_TAB_KIND,
					// 不声明 priority：默认 extension，即外部插件本就在的最高 band。
					title: () => "修改审查",
					guide: [{
						order: 60,
						title: () => "修改审查",
						description: () => "按会话查看 write / edit 改动，可逐项撤回",
						icon: ReviewTabIcon
					}]
				}), "diff-review: right sidebar tab type");
				sctx.slots.inject("sidebar.right.pane.tab", () => sctx.slots.register(
					{ name: "sidebar.right.pane.tab", key: RIGHT_TAB_ID },
					(props) => React.createElement(ReviewView, {
						sessionId: props.sessionId,
						openFile: (path, cwd) => openFileFor(props.sessionId, path, cwd)
					})));
				sctx.slots.inject("sidebar.right.pane.tab.title", () => sctx.slots.register(
					{ name: "sidebar.right.pane.tab.title", key: RIGHT_TAB_ID },
					(props) => React.createElement("span",
						{ style: { display: "inline-flex", alignItems: "center", gap: 6 } },
						React.createElement(ReviewTabIcon, { size: 14 }),
						props.useTabInfo().tab.title)));
			});
