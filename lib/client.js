	...
	factory: (require) => {
		...
		function openFileFor(sessionId, path) {
			try {
				...
				let abs = path
				if (rtApi && rtApi.resolveWorkspacePath) { abs = rtApi.resolveWorkspacePath(cwd, path) }
				else if (cwd && typeof path === "string" && !path.startsWith("/") && !/^[a-zA-Z]:[\\/]/.test(path)) {
					abs = cwd.replace(/[\\/]+$/, "") + "/" + path.replace(/^[\\/]+/, "").replace(/^[\\/]+/, "")
				}
				if (ctxRef.workspaces && ctxRef.workspaces.openPath && abs) {
		...
		function TurnReview({ matched, sessionId, turn: turnLoc, seq, openFile }) {
			...
			const showCtx = (e, path) => {
				e.preventDefault(); e.stopPropagation();
				setMenu({ x: e.clientX, y: e.clientY, items: [
					{ label: "打开文件", run: () => { if (openFile) openFile(path); } },
